            ...
            <form action="{{ route('booking', $flight->flight_number) }}" id="Tiers" class="grid grid-cols-2 gap-x-[30px]">
                ...
                @foreach ($flight->classes as $class)
                    <div class="flex flex-col h-fit rounded-[20px] p-5 pb-[30px] gap-5 bg-white">
                        ...
                        @endforeach
                        <button class="w-full rounded-full py-3 px-5 text-center bg-garuda-blue hover:shadow-[0px_14px_30px_0px_#0068FF66] transition-all duration-300" onclick="document.getElementById('flight_class_id').value= '{{$class->id}}'">
